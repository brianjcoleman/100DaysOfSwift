...
//  Milestone9
//
//  Created by Brian Coleman on 2022-05-31.
//
...
class ViewController: UIViewController, UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    func setDefaultImage() {
        ...
        let image = renderer.image { ctx in
            let defaultText = "Import a photo from your photo library"
    ...
    func showCaptionAlert(for position: Position) {
        var title = ""
        if position == .top { title = "Top Text" }
        else if position == .bottom { title = "Bottom Text" }
